...
<main id="main" class="main">
    <div class="card">
        ...
        @endif
        <div class="card-body">
            <h5 class="card-title">Edit Jenis Lapangan Form</h5>

            <!-- Floating Labels Form -->
            <form action="{{ route('Jenis.update', $j->idjenis) }}" method="POST" class="row g-3">
                @csrf
                @method('PUT')
                <div class="col-md-6">
                    <div class="form-floating">
                            <input type="text" class="form-control" id="floatingName" placeholder="IDJenisLapangan" value="JNS{{$j->idjenis}}" disabled>
                            <input name="idjenis" value="{{$j->idjenis}}" type="text" hidden>
                        <label for="floatingName">ID Jenis Lapangan</label>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="form-floating">
                        <input type="text" class="form-control" id="floatingName" placeholder="JenisLapangan" name="jenis" value="{{$j->jenis}}">
